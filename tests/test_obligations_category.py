...
def test_new_install_joppd_is_place(spine):
    t = obligations.get_type(spine, "JOPPD")
    assert t["label"] == "Plaće" and t["category"] == "place"


def test_other_types_have_empty_category(spine):
    assert obligations.get_type(spine, "PDV")["category"] == ""


def test_upsert_type_persists_category(spine):
    obligations.upsert_type(spine, "NAJAM", "Najam", "monthly:10", "monthly", "manual",
                       category="najam", user="a")
    assert obligations.get_type(spine, "NAJAM")["category"] == "najam"


def test_list_types_includes_category(spine):
    types = {t["kind"]: t for t in obligations.list_types(spine)}
...
def test_upsert_without_category_preserves_existing(spine):
    ...
    obligations.upsert_type(spine, "JOPPD", "Plaće", "monthly:15", "monthly", "employees")
    assert obligations.get_type(spine, "JOPPD")["category"] == "place"
    # eksplicitan prazan string ipak briše
    obligations.upsert_type(spine, "JOPPD", "Plaće", "monthly:15", "monthly", "employees", category="")
    assert obligations.get_type(spine, "JOPPD")["category"] == ""
...
def test_upsert_rejects_invalid_rule(spine):
    ...
    for bad in [("monthly", "monthly:garbage"), ("yearly", "yearly:99-99"),
        ...
        with pytest.raises(ValueError):
            obligations.upsert_type(spine, "X", "X", bad[1], bad[0], "all_active")
    # valjana prolaze
    obligations.upsert_type(spine, "Y", "Y", "yearly:02-29", "yearly", "all_active")
    assert obligations.get_type(spine, "Y")["rule"] == "yearly:02-29"
