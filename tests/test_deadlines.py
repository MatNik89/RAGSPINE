...
def test_next_workday_skips_weekend_and_holiday():
    ...
    assert deadlines.next_workday(date(2026, 8, 15)) == date(2026, 8, 17)
    # običan radni dan ostaje isti
    assert deadlines.next_workday(date(2026, 8, 20)) == date(2026, 8, 20)


def test_due_for_month_rules():
    assert deadlines.due_for_month("monthly:20", 2026, 8) == date(2026, 8, 20)
    assert deadlines.due_for_month("quarterly:20", 2026, 8) is None   # kolovoz nije kvartalni
    assert deadlines.due_for_month("quarterly:20", 2026, 7) == date(2026, 7, 20)
    assert deadlines.due_for_month("yearly:02-28", 2026, 2) == date(2026, 2, 28)
    assert deadlines.due_for_month("yearly:02-28", 2026, 3) is None
    # dan preko kraja mjeseca se skraćuje
    assert deadlines.due_for_month("monthly:31", 2026, 2) == date(2026, 2, 28)
...
def test_generate_idempotent(spine):
    with spine.write() as c:
        c.execute("INSERT INTO clients(name,oib,pdv_status,active) VALUES('A','1','u sustavu pdv',1)")
    deadlines.generate(spine, months_ahead=3, today=date(2026, 8, 2))
...
def test_rokovi_job_registered(spine, cfg):
    ...
    assert "deadlines" in {j.name for j in sched.jobs}
...
def test_pdv_rok_2026_zadnji_dan_mjeseca():
    ...
    assert deadlines.due_for_month("monthly:31", 2026, 2) == date(2026, 2, 28)
    assert deadlines.due_for_month("monthly:31", 2026, 4) == date(2026, 4, 30)
    assert deadlines.due_for_month("monthly:31", 2026, 8) == date(2026, 8, 31)
