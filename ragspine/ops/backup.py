...
def verify_backup(path: str) -> bool:
    """Valjan RAGSPINE snapshot: postojeća datoteka, quick_check ok, i sadrži
    RAGSPINE shemu. Otvara read-only da sqlite3.connect NE stvori praznu bazu na
...
def restore_backup(cfg, path: str) -> dict:
    ...
    hardening: verificira RAGSPINE shemu (ne samo quick_check), konzistentan
    ...
    if not verify_backup(path):
        raise ValueError("nije valjana RAGSPINE baza (nepostojeća / prazna / kriva shema)")
    db = cfg.db_path
